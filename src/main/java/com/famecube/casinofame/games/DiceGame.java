...
public class DiceGame {
    ...
    public DiceGame(FileConfiguration config, FairnessManager fairnessManager, ChipManager chipManager, SoundUtil soundUtil, boolean particles) {
        this.config = config;
    ...
    private void finish(GameSession session, long bet, long seed, int total) {
        Player player = session.getPlayer();
        String choice = session.getSelection();
        long payout = 0L;
        boolean win = false;
        String outcome = String.valueOf(total);
        if ("UNDER".equals(choice) && total < 7) {
            payout = Math.round(bet * config.getDouble("dice.payoutUnderOver", 2.0));
            win = true;
        } else if ("OVER".equals(choice) && total > 7) {
            payout = Math.round(bet * config.getDouble("dice.payoutUnderOver", 2.0));
            win = true;
        } else if ("EXACT".equals(choice) && total == 7) {
            payout = Math.round(bet * config.getDouble("dice.payoutExact", 5.0));
            win = true;
        }
        if (win) {
        ...
        fairnessManager.recordRound(roundId, player.getName(), player.getUniqueId(), "DICE", bet, payout, seed, outcome);
        if (win) {
